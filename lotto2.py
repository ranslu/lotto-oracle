...
def fetch_daily_grand():
    ...
    def try_parse_date(txt):
        for fmt in ["%d %b %Y","%B %d, %Y","%b %d, %Y","%Y-%m-%d",
                    "%A, %B %d, %Y","%d/%m/%Y","%m/%d/%Y","%b %d %Y"]:
            try:
                return datetime.strptime(txt.strip(), fmt).strftime("%Y-%m-%d")
            except ValueError:
                continue
        # Try regex for date like "Apr 23 2026"
    ...
    try:
        ...
        while i < len(all_tds):
            ...
            # Look for date pattern like (2026-04-23
            date_match = re.search(r"\((\d{4}-\d{2}-\d{2})", txt)
            if date_match:
                draw_date = date_match.group(1)
                # Get numbers from li items in this td or next td
                lis = td.find_all("li")
                ...
                if len(nums) >= 6:
                    ...
                    main = sorted(nums[:-1])
                    if len(main) == 5:
                        draws.append((draw_date, main, grand))
            i += 1
